...
/*   Updated: 2022/09/01 03:20:57 by yjarhbou         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int ft_export(t_data *data, char **cmds)
{
    ...
    //g_var.exit_status = 0;
    if (cmds[1] == NULL)
        return (ft_print_env(data->envp));
    else
    {
        while (cmds[i])
        {
            if (ft_is_identifier(cmds[i]))
                ft_add_to_env(data->env, cmds[i]);
            else
            {
                //g_var.exit_status = 1;
